...
function run() {
  ...
  for (var i=0;i<downloadCodeList.length;i++) {
	var titu = document.title.split(" - YouTube")[0]; 
	if (titu.indexOf("(") == 0) { 
		titu = titu.split(/\)(.+)/)[1];
		titu = titu.replace(" ", "");
	}
	...
	listLink.setAttribute('onclick','javascript:var entra = document.createElement("input"); entra.setAttribute("type", "text"); entra.setAttribute("value","' + titu + "." +FORMAT_TYPE[downloadCodeList[i].format]+ '"); entra.setAttribute("id","zeta"); document.body.appendChild(entra); var ttt = document.getElementById("zeta"); ttt.select(); document.execCommand("Copy"); document.body.removeChild(entra);');
...
function botao() {
	...
	Link.setAttribute('onclick', 'javascript: if (document.getElementById("zetaframe") !== null) {document.getElementById("zetaframe").remove();} if (document.getElementById("zetinha") === null) {insere();}');
    var listButton=document.createElement('span');
    listButton.setAttribute('class', 'yt-uix-button-menu-item');
	...
	function injetar() 
	{
		...
		script1.src = 'https://sites.google.com/site/geradorzeta/hospedagem/insere.js?attredirects=0&d=1'; 
